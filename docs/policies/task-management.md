...
## 方針
- Plan を提示して承認を得たあと、実装着手前に `docs/todo/` 配下へ ToDo ファイルを作成し、作業ごとの進捗ログを残す。フロントマターには `目的`・`関連ブランチ`・`関連Issue`・`roadmap_item` を必ず記載し、`roadmap_item` には `RM-xxx テーマ名` を記入する。`RM-000` はロードマップ整備・横断タスク専用とし、実装テーマへは固有の `RM-xxx` を割り当てる。
- コードへの影響がなく、ユーザーから明示的に ToDo 作成不要と指示された整備タスクは例外とし、ToDo の新規作成を省略してよい。判断に迷う場合はユーザーへ作成要否を確認する。
- ファイル名は `YYYYMMDD-rmxxx-<slug>.md` とし、対象ロードマップ番号を含めて日付と作業内容が一目で分かるようにする（例: `20251122-rm070-dev-process-guidance.md`）。既存の旧形式はアーカイブに残すが、新規作成時は必ず RM 番号を付与する。
- タスクはチェックボックス形式で記述し、「誰が何をどこまで行うか」が分かる粒度で記載する。
- メモ欄は判断材料やリスクなどタスク本文に収まりきらない補足に限定し、進捗更新はチェックボックス側で行う。ただし「計画策定」チェック項目のメモ欄は、承認済み Plan を全文転記する目的で詳細記述を許容する。
- チェック項目の対象を確認した結果「更新不要」と判断した場合でも、実査完了としてチェックを付け、その判断理由をメモに残す（例: `docs/runbook は既存手順と差分なし`）。
- チェックリストに入れ子構造を採用する場合、子タスクをすべて完了にした時点で親タスクのチェック状態も確認し、未完のままであれば `[x]` へ更新する。親タスクのメモ欄には進捗の要約を残し、階層間の整合を保つ。
- 完了後も PR から参照できるようにし、継続的なナレッジとして `docs/todo/archive/` に保管する。
- コミットは小さな単位で積み重ね、変更意図が追跡できる履歴を維持する。
- Plan／ToDo の記述では、目的・参照資料・完了条件を簡潔にまとめ、詳細は `docs/policies/context-engineering.md` が示す下位ドキュメントへ誘導する。必要なリンクはメモ欄に明示する。
...
4. 新しいブランチを `feat|fix|chore|docs/rmxxx-<slug>` 形式で作成し、ブランチ名を ToDo の `関連ブランチ` に記載する。`rmxxx` には対象ロードマップ番号（例: `docs/rm070-dev-process-guidance`）を含める。
5. ブランチ作成後に作業を開始し、途中で main ブランチ上に直接コミットを作らない。

## Approval-First Development Policy
- **Plan 提示の必須化**: バグ修正・機能追加を問わず、実装前に scope／対象ファイル・モジュール／想定影響ファイル／リスクと前提／テスト戦略（単体・統合など）／ロールバック方法を箇条書きでまとめた Plan を提示し、ユーザーの承認を得るまで実装を開始しない。承認後は Plan の記載内容を該当 ToDo の「計画策定」チェック項目のメモ欄へそのまま転記し、Plan と ToDo の内容を常に一致させる。
...
- **HITL 承認との連携**: stage 3・4 の Human-in-the-Loop 承認作業では、Plan と ToDo に承認対象（`prepare_card.json` / `draft_approved.json`）と必要なレビュー項目を明記し、承認ログ（メッセージ ID）を残す。承認 UI 上の差戻しや部分承認が発生した場合は ToDo やノートに記録しておく。
