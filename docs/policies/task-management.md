...
- ファイル名は `YYYYMMDD-rmxxx-<slug>.md` とし、対象ロードマップ番号を含めて日付と作業内容が一目で分かるようにする（例: `20251122-rm069-dev-process-guidance.md`）。既存の旧形式はアーカイブに残すが、新規作成時は必ず RM 番号を付与する。
...
4. 新しいブランチを `feat|fix|chore|docs/rmxxx-<slug>` 形式で作成し、ブランチ名を ToDo の `関連ブランチ` に記載する。`rmxxx` には対象ロードマップ番号（例: `docs/rm069-dev-process-guidance`）を含める。
...
- **HITL 承認との連携**: 工程 3・4 の Human-in-the-Loop 承認作業では、Plan と ToDo に承認対象（`prepare_card.json` / `draft_approved.json`）と必要なレビュー項目を明記し、承認ログ（メッセージ ID）を残す。承認 UI 上の差戻しや部分承認が発生した場合は ToDo やノートに記録しておく。
