...
## 入力
- `brief_cards.json`: `cards[*]` に `card_id`, `chapter`, `message`, `narrative`, `supporting_points`, `story.phase`, `intent_tags` を保持。すべてのカードは `status`=`approved` であることが前提。
- `brief_log.json`: HITL 操作ログ。差戻し理由、承認履歴、AI 再生成履歴を参照する。
- `ai_generation_meta.json`: 生成メタデータ。カード統計や入力ハッシュをメタ情報として利用する。
- `jobspec.json`: テンプレ構造定義。`slides[*]` に `layout`, `title`, `textboxes` / `tables` / `images` / `charts` / `textboxes` など anchor 情報を含む。
- `layouts.jsonl`（任意）: レイアウト用途タグや収容目安。AI 推薦およびヒューリスティック補助に利用。
- `analysis_summary.json`（任意）: 工程6 Analyzer からの差戻し情報。レイアウト整合や重大度別件数を参照し、割当優先度を調整する。
- 章テンプレ辞書（任意）: `config/chapter_templates/*`。章必須枚数や構成スコア評価に利用する。
- 差戻し理由テンプレ辞書（任意）: `config/return_reasons.json`。HITL 差戻し操作で利用。
