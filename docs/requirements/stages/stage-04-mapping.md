...
- 最終成果物は工程5（レンダリング）が直接受け取る `rendering_ready.json` と監査ログ群。

## サブ工程と成果物
| サブ工程 | 区分 | 主な成果物 | 概要 |
| --- | --- | --- | --- |
| 4.1 ドラフト構成 | HITL | `draft_approved.json`, `draft_review_log.json`, `draft_meta.json` | 章構成・スライド順・`layout_hint` を確定し、承認ログとメタ情報を記録する。 |
| 4.2 レイアウトマッピング | 自動 | `rendering_ready.json`, `mapping_log.json`, `fallback_report.json` | テンプレ構造とドラフト情報を統合し、プレースホルダ割付とフォールバック履歴を確定する。 |
...
### 出力
- `rendering_ready.json`: スライドごとの `layout_id`、プレースホルダ割付、`job_meta` / `job_auth` など。
...
5. JSON スキーマ検証を通過した `rendering_ready.json` を出力し、工程5へ送る。

### 品質ゲート
- 全スライドが `layout_id` を持ち、必須プレースホルダが埋まっている。
- `rendering_ready.json` がスキーマ検証を通過し、空要素は意図的であるとログに記載されている。
...
- 監査ログ (`audit_log.json`) で `rendering_ready`・`mapping_log` の SHA-256 を収集し、`mapping_meta` にスライド数・フォールバック対象 ID・生成時刻を記録する。
...
### CLI 支援
- `pptx mapping` コマンドは `rendering_ready.json`・`mapping_log.json`・必要に応じて `fallback_report.json` を生成する。
