...
- CLI からは `uv run pptx gen .pptx/gen/generate_ready.json --branding config/branding.json` を利用して工程5のみを実行する。工程4の成果物を事前に確認したい場合は `pptx compose` や `pptx mapping` を使用する。
