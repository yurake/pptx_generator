...
- CLI では `uv run pptx mapping <spec.json> --prepare-cards .pptx/prepare/prepare_card.json` が工程5の基本パス。ログやメタのパスは `prepare_card.json.meta` から自動で解決される。HITL とまとめて再実行する場合は `pptx compose` を利用し、レンダリングは工程3で得た `generate_ready.json` を入力に `pptx gen` で実行する。
