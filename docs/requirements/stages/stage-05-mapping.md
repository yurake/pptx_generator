...
- `prepare_card.json`: 章・メッセージ・意図タグを含むプレペアカード。
...
5. JSON スキーマ検証を通過した `generate_ready.json` を出力し、stage 6 へ送る。
...
- Analyzer 指摘が `mapping_log.json` のメタ（件数サマリ）およびスライド単位サマリに反映され、後続 stage で参照できる。
...
- Analyzer は stage 6 で走査した結果を再度 `mapping_log.json` に書き戻し、フォールバック履歴と合わせて可観測性を確保する。
...
- CLI では `uv run pptx mapping <spec.json> --prepare-cards .pptx/prepare/prepare_card.json` が stage 5 の基本パス。ログやメタのパスは `prepare_card.json.meta` から自動で解決される。HITL とまとめて再実行する場合は `pptx compose` を利用し、レンダリングは stage 3 で得た `generate_ready.json` を入力に `pptx gen` で実行する。
