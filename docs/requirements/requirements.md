...
## 3. 利用者とユースケース
- 主利用者: このツールを導入する組織のパワーポイント利用者全般（社内外を問わず）。社内テンプレ運用の場合は社内データ前提、外部導入時は各組織のテンプレ・ポリシーに合わせる。
- 想定ユースケース
  - 資料データを渡し、1分以内に提案書を生成して即時レビュー開始。
  - 急な会議や緊急の提案依頼にも待ち時間なく対応。
...
- サブタイトル・ノート・テキストボックスではテンプレートが定義するフォント／段落スタイルを適用し、アンカー指定時はテンプレート図形名を維持して後続 stage が再参照できること。
- レイアウト別のスタイル（フォント、配色、フォールバック配置、段落揃え・行間・余白・インデント）はテンプレート（スライドマスター／テーマ）の情報から自動抽出し、レンダラーが忠実に反映すること。Polisher での仕上げはフォールバック対応と監査ログ出力に留め、段落スタイルの補正は原則レンダリング stage で完結させる。
...
### 4.7 HITL 承認フロー
- stage 3（コンテンツ準備）と stage 4（ドラフト構成設計）で、人による承認・差戻し・部分承認が可能であること。
- 承認ログには `slide_id` / `action` / `actor` / `timestamp` を記録し、後続 stage とトレーサブルに連携できること。
...
### 4.9 テンプレ構造解析
- `.pptx` テンプレートからレイアウト構造を抽出する CLI (`extract-template`) を提供し、stage 2 を支援すること。
- 抽出結果には `layout_id`, `placeholders`, `text_hint`, `usage_tags`, `media_hint` を含め、stage 3〜5 が参照できること。
- 抽出処理はテンプレ依存カタログ (`jobspec.json`) を同時生成し、レイアウトごとのアンカーや推奨用途を後 stage が共有できるようにすること。
- usage_tags の推定は生成 AI（Template AI）を標準で起動し、`src/pptx_generator/config/usage_tags.json` に定義した canonical タグと説明文をプロンプトへ渡して整合性を維持すること。`PPTX_TEMPLATE_LLM_PROVIDER=mock` の場合のみ静的ルールで完結できるようフォールバックを維持すること。
