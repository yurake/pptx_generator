...
## 次ステップ（ロードマップ項目候補）
- 上記の見直し項目（工程2のジョブスペック雛形生成、工程2抽出＋検証のラッパー化、工程3/4の役割再定義、工程4+5ラッパーコマンド、`pptx gen` のスコープ再調整など）は、それぞれロードマップ項目（RM）として切り出す見込み。今後 Plan 作成時に RM 番号を割り当て、`docs/roadmap/roadmap.md` へ反映する。
