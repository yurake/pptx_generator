...
> **更新情報 (2025-11-03)**: 現行の工程構成は 4 工程（テンプレ工程 → コンテンツ準備 → マッピング → レンダリング）へ再編されました。本メモの 5 工程言及は当時の検討内容として参照してください。
...
| 3 コンテンツ準備 | HITL | 入力データをスライド素材へ変換し確定 | Markdown/CSV/JSON 等、ブリーフ | 承認済コンテンツ JSON | 自動案を提示し、人が文面・意図タグを確定。差戻しは再生成ログに記録。 |
...
> **最終構成（当時案）:** 5 工程で循環し、3・4 のみ人の承認を必須とする。承認済みデータ以外は自動工程に流さない。
