...
> **更新情報 (2025-11-03)**: 現行の工程構成は 4 工程（テンプレ工程 → コンテンツ正規化 → マッピング → レンダリング）へ再編されました。本メモの 5 工程言及は当時の検討内容として参照してください。
...
| 4 マッピング (HITL + 自動) | HITL + 自動 | 章構成承認と PH マッピング | 承認済コンテンツ JSON、章テンプレ、レイアウト構造 JSON | `draft_approved.json`, `rendering_ready.json` | HITL が章構成・layout_hint を確定し、自動マッピングがプレースホルダ割付・フォールバック制御を担当。 |
