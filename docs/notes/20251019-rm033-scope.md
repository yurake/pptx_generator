...
- 直近のリファクタで `pptx compose`（工程4一括）、`pptx mapping`（工程4後半）、`pptx render`（工程5）が追加済み。`pptx render` は `rendering_ready.json` を唯一の主要入力とし、`rendering_ready_to_jobspec()` を介して JobSpec を再構築する。
