...
## 現状確認
- CLI `pptx gen` は工程3〜6を一括実行しつつ、内部で `BriefNormalizationStep`（工程3）→`DraftStructuringStep`（工程4）→`MappingStep`（工程5）→`SimpleRendererStep` などを順に呼び出している。
...
- 工程3/4は CLI として独立しておらず、`pptx mapping` 実行時にも `BriefNormalizationStep` と `DraftStructuringStep` が走る（指定がなければスキップする構成）。
- `BriefNormalizationStep` は Brief 成果物とログを読み込むだけで生成処理は持たず、HITL で整備済みのファイルを前提にしている。
- `DraftStructuringStep` は `brief_document` アーティファクトと `layouts.jsonl` を元に `draft_draft.json` / `draft_approved.json` を生成し、`DraftStore` へも登録するため、工程4独立実行時の副作用 (draft 出力とメモリ内ストア) を想定済み。
...
- 想定コマンド: `pptx content` でブリーフ入力から `brief_cards.json` を生成し、`pptx outline` で `brief_cards.json` から `draft_approved.json` を生成。
