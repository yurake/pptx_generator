...
> 注記: 工程5の CLI は 2025-11 時点で `pptx gen` に統一されており、本メモに登場する `pptx render` は検討当時の案です。
...
- サンプルファイルの構造差分
  ...
  - `samples/prepare/prepare_card.json` は工程3の成果物を保持し、カードごとに `role.story_phase` / `role.intent_tags` / `content.title` / `content.body[]` / `content.notes[]` を含むが、レイアウトアンカーなどテンプレ依存情報は含まない。
  - 差分の意味: 工程3以降で同じ `slide_id` をキーに統合するため、ジョブスペック側にテンプレと整合する構造情報が必要になる。
- ジョブスペックが参照される工程
  - 工程3: `uv run pptx prepare ...` がジョブスペックを参照しつつプレペア成果物を生成する際のベースになる (`README.md:96-109`)。
...
- ドキュメント記載との整合
  - `docs/requirements/stages/stage-03-content-normalization.md` はジョブスペックを工程3の入力（プレペア整形済みデータ）と位置付け、HITL 承認後に `prepare_card.json` を生成する流れを定義。
  - `docs/design/design.md:34-60` も「工程3: プレペア正規化（HITL）が `spec.json` とテンプレ構造を参照しつつ `prepare_card.json` を作る」前提でアーキテクチャ図を記載しており、ジョブスペックは工程3より前に人が整備する想定になっている。
...
- 工程4 (生成AI) の役割再定義
  - 工程3で作成したプレペア（抽象カード）と、工程2のジョブスペック雛形／テンプレ構造 JSON をマージし、`layout_hint` やページ割り当てまで具体的に埋めた `draft_approved.json` を生成する。
...
- RM-046: 工程3で生成AIが案件プレペアを整理するワークフロー整備。
- RM-047: 工程4でプレペアとテンプレ構造 JSON をマージし、`layout_hint` 付きドラフトを生成するテーマ。
