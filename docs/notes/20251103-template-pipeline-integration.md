# 2025-11-03 テンプレ 統合集約メモ

## 背景
- ロードマップ項目 **RM-051 テンプレ 統合集約** に基づき、旧 stage 1（テンプレ準備）と旧 stage 2（テンプレ構造抽出）を統合し、新しい「stage 1: テンプレ」として再定義した。
- CLI 利用者には `uv run pptx template` を標準ルートとして案内し、従来の `tpl-extract` / `layout-validate` / `tpl-release` は詳細オプションに位置付ける。
- 4 stage 体系（テンプレ → コンテンツ準備 → マッピング → レンダリング）へ移行するにあたり、主要ドキュメント・テスト・サンプルを更新した。
...
- `docs/design/stages/stage-01-template.md` の詳細化（抽出ワークフロー図、例外ハンドリングの整理）。
