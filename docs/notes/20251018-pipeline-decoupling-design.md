...
> **補足:** 現在は `pptx gen` を stage 5 専用コマンドとして提供しており、本メモは再定義前の検討内容を記録したものです。

## 背景
- `pptx gen` が stage 3〜5（Content Approval → Mapping → Rendering/Analyzer/PDF）を一括実行しており、stage 5 のみの再実行や stage 4 成果物の再利用ができない。
- 最終像では各 stage を独立 CLI として扱い、stage 4 の成果物（`generate_ready.json`）を stage 5 の唯一の入力にしたい。
- ユーザーとの冒頭議論で、stage 3/4の成果物は stage 4 に反映済みであり、stage 5 には `generate_ready.json` だけを渡せる構成が望ましいと確認した。

## 目的
1. stage 4（Mapping）と stage 5（Rendering/Analyzer/PDF）を分離し、個別に実行・再実行可能にする。
...
### データモデル
- `GenerateReadyMeta` に `job_meta` と `job_auth` を追加し、元の `JobSpec` メタ情報を保持。stage 5 での再構成に使用する。
...
- CLI オプションの整理（特に `--content-approved` など stage 3/4向けフラグ）は段階的に deprecate 予定。今回の対応では互換性維持のみ。
