...
> **更新情報 (2025-11-03)**: README は 4 stage 構成（テンプレ stage → コンテンツ準備 → マッピング → レンダリング）へ更新済み。以下のメモは 5 stage 体制を前提とした当時の改善案として参照する。

`README.md` の現状と改善案を整理する。主な論点は「一般的に記載すべき章構成の棚卸し」と「アーキテクチャ概要（5 stage）と使い方の整合」である。

## 現状整理

| セクション | 現在の内容 | 課題・不足 | 対応方針 |
| --- | --- | --- | --- |
| プロジェクト概要 | 冒頭にツールの目的と特徴を簡潔に記載 | 主要機能（HITL 必須 stage、テンプレ構造抽出、PDF 変換など）の整理が不足 | 冒頭で提供価値とコア機能を箇条書きで明示 |
| アーキテクチャ概要 | 5 stage を説明し、参照ドキュメントを案内 | stage と CLI コマンドの対応が README 内では不明瞭 | 後続の「使い方」で stage と具体操作を対応付け |
...
| 使い方 | `pptx gen` と `tpl-extract` を中心にコマンド列挙 | 5 stage のうち HITL や計画中機能との関係が不明。分析結果の出力場所が断片的 | stage ごとに「現在の進め方」「対応コマンド」「HITL/計画中」を整理。`analysis.json` など出力整理 |
...
1. **全体フローの把握**: `docs/design/design.md` を参照し、stage 1〜6 の位置づけを理解する。
2. **stage 別の操作ガイド**
   - stage 1: テンプレ準備（手動／既存資産管理）— 現状はテンプレ管理ポリシーを参照する。CLI 対応は計画中である旨を明記。
   - stage 2: `uv run pptx tpl-extract` でテンプレ構造抽出。出力物（レイアウト JSON、`branding.json`）と保存先を説明。
   - stage 3: コンテンツ準備（HITL）。`docs/requirements/stages/stage-03-...` など参照し、ワークシート例を記載。自動化は今後の検討と記載。
   - stage 4: ドラフト構成設計（HITL）。`draft_approved.json` の作成例と進行状況を説明。
   - stage 5: `generate_ready.json` を生成するマッピング stage（自動）。現状は CLI 実装中で、手順は設計ドキュメントを案内。
   - stage 6: `uv run pptx gen` 実行で PPTX/PDF を生成。出力ディレクトリとログを整理し、`--export-pdf` オプションや LibreOffice 依存を明記。
