...
- `src/pptx_generator/cli.py` の `pptx content` コマンド（旧実装）は `ContentAIOrchestrator` が `JobSpec.slides` から `ContentSlide` を生成し、`content_draft.json` を出力していた。
...
- スキーマ更新時は `docs/design/schema/stage-03-content-normalization.md` と `docs/requirements/stages/stage-03-content-normalization.md` を同時更新し、`samples/` 配下に BriefCard 前提のサンプル（例: `samples/brief/brief_cards.sample.jsonc`）を追加する。

## CLI インターフェース検討
- 要望: `uv run pptx content <brief file>` でブリーフ入力を直接指定し、従来必須だった `spec.json` 引数を不要化する。
- 影響:
...
- `samples/`: BriefCard 用のサンプルセット（`brief_cards.sample.jsonc` など）を追加し、旧コンテンツ承認系のサンプルは `archive/` へ退避する方針をドキュメントに記載。

## 実装ロードマップ（案）
1. **Stage3 基盤更新**
   - `BriefCard` モデルと関連スキーマを実装し、CLI `pptx content` / API / パイプライン（`ContentAIOrchestrator`, `ContentImportService`, `ContentApprovalStep`）を全て新モデルへ置換。
   - `uv run pptx content <brief file>` をエントリに据え、旧 `spec_path` 引数と `ContentSlide` 依存コードを削除。
...
- CLI 統合: `uv run pptx content samples/contents/sample_import_content_summary.txt` を基準に生成物（`brief_cards.json`, `ai_generation_meta.json`, `brief_log.json` など）を検証。
