...
- Analyzer の出力を stage 1〜6 へどのように組み込めるか検討した結果を整理し、既存ドキュメントや計画への反映状況を確認する。
- 今回の検討では、各 stage で「設計・要件にすでに記載されている内容」と「未反映だが有効な拡張案」を切り分けた。
...
### stage 5 PPTX レンダリング
- 既存ドキュメント  
  - RM-013 でアナライザー実装が完了し、stage 6 後段で解析する方針はロードマップに記載済み（`docs/roadmap/roadmap.md:154-161`）。
...
## 次のアクション候補
- 追加アイデアを stage 別ロードマップ項目へ反映するためのタスク発行（例: RM-022, RM-024, RM-025, RM-026 の補強）。
- Analyzer 結果の共通インターフェース設計（API / CLI / ファイル連携）と、各 stage での取り込みポイントを図式化した設計資料の作成。
