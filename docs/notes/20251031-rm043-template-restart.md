...
4. サンプル JSON 整備:
   - `samples/json/sample_spec.json` をレイアウト構成（Two Column Detail アジェンダ、KPI チャート追加など）に合わせて更新。
   - `samples/json/sample_spec_extended.json` を新規作成し、50 ページ分のレイアウト例を定義。
...
6. CLI 実行と検証:
   ```bash
   UV_CACHE_DIR=.uv-cache uv run pptx gen samples/json/sample_spec_extended.json \\
     --template samples/templates/templates.pptx \\
