...
## 背景
- 工程3 の HITL 承認基盤を `ContentSlide` から `PrepareCard` へ移行するため、既存メモを刷新。
...
- **PrepareNormalizationStep**: パイプライン内で PrepareCard 集合をロードし、工程4/5 へ `PrepareDocument` と `PrepareStoryOutline` を提供するステップ。
...
- `PrepareNormalizationStep` が `PipelineContext` に `prepare_document`, `prepare_story_outline`, `prepare_log`, `ai_generation_meta` を登録。工程4/5 は `PrepareCard` 情報を直接参照する。
