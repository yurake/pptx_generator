...
- [x] stage 3 出力 (`prepare_card.json`) に必要なストーリー項目が要件として明記されている
- [x] stage 4 の整合チェック手順に `story_outline` との突合が含まれている
- [x] Runbook に運用・ロールバック手順が追加されている
- [x] ToDo から参照できるブランチ情報・ロードマップ番号が更新されている

## 所見
- 現時点で stage 3→4 の受け渡し仕様は文書化されている。骨子バージョン差異時のエスカレーションは Runbook へ追記済み。
...
- 将来的に stage 4 の UI を構築する際は、Runbook のレビュー観点に操作ログ検証を追記する。
