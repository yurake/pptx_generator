...
- **運用ポイント**
  - テンプレ作成（stage 1）では PPTX 本体のみ整備し、メタ抽出は stage 2 に委譲。
...
- **目的**: ブランドごとの PPTX テンプレ資産を整備し、後 stage がそのまま利用できる状態にする。
...
- **制約・フォールバック**:
  - メタデータ（レイアウト構造・用途タグ）は出力しない。抽出は stage 2 で実施。
  - 後段の機械抽出が可能なように、PH はマスター階層に配置し、不要図形を作らない。
  - 版管理はファイル名で行い、互換性検証は stage 2 で対応。
- **主要 I/F**: 受け渡しは PPTX 本体のみ。差分通知はテンプレ版の比較レポートとして別途生成。
- **完了基準**: 想定レイアウトがすべて盛り込まれ、崩れのない PPTX が用意されていること（stage 2 で 100% 抽出可能）。

### 2. テンプレ構造抽出
- **オペレーション**: 自動
- **目的**: PPTX からレイアウト構造・用途タグ・ヒント値を機械抽出し、後 stage が利用できる JSON を作る。
- **入力**: テンプレ PPTX、本 stage 用の抽出ルール（PH 種別マッピング、面積→ヒント係数など）。
- **出力**:
...
- **入力**:
  ...
  - テンプレ構造情報（stage 2 出力の用途タグや layout ヒント）。
- **出力**:
...
- **入力**:
  - `prepare_card.json`（stage 3 の承認結果）。
  - テンプレ構造情報（stage 2 出力）。
...
- **処理概要**:
  ...
  4. 承認済みスライドはロックされ、stage 5 では `draft_approved.json` のみ利用。
- **制約・フォールバック**:
...
- **主要 I/F**:
  - `draft_draft.json` ⇄ UI。
  - `draft_approved.json`（stage 5 の入力）。
...
- **入力**:
  ...
  - `layouts.jsonl` / `diagnostics.json`（stage 2 出力）。
...
- **主要 I/F**: `generate_ready.json`（stage 5 の唯一の入力）、`mapping_log.json`（監査・検証用）。
- **完了基準**:
...
- **ログ**: 承認イベント（誰がいつ何を確定）と差戻し理由を記録し、slide_uid で stage 間トレース。
...
- **テンプレ依存スタイル**
  - フォント・色・段落はテンプレ側で完結。後 stage でスタイルを上書きしない。
- **出力の揺らぎ**
