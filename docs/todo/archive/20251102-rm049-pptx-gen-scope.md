...
- [x] 計画策定（スコープ・前提の整理）
  - メモ:
    ...
    - 追加Plan（template サブコマンド維持）
      - **スコープ**: `pptx template` サブコマンドを `feat/rm049-pptx-gen-scope` の CLI 構成へ再統合し、テンプレ の一括実行（抽出・検証・リリース）を継続提供する。generate_ready 統合後の共通処理と競合しないよう調整し、ドキュメントとテストも復元する。
      - **主な作業**
...
- **2025-11-04 CLIテンプレートコマンド復活**
  - `pptx template` サブコマンドを再実装し、抽出・検証・リリースの一括実行を復旧。CLI 共通ヘルパーを整理し、テンプレ のサブコマンド群と競合しない形で再統合。
  - README・CLI リファレンス・設計ドキュメントを `template` 前提の手順に差し戻し、テンプレ の説明を最新化。
