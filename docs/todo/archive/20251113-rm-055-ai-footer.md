...
- [x] 実装（Polisherへのフッタ付与機能追加）
  - メモ: 実装完了（2025-11-13）。python-pptxを使用してフッタテキストボックスを追加
  - [x] Polisherステップへのフッタ付与ロジック追加
  - [x] AI生成メタデータの判定処理
  - [x] フッタ文言の生成・配置処理
  - 注記: 現在は全スライドにフッタを付与する仮実装。将来的にはslide_idマッピングを改善予定
- [x] テスト（単体・統合テスト追加）
  - メモ: テスト完了（2025-11-13）。全テストパス確認済み
  - [x] 単体テスト（フッタ付与ロジック）
    - test_ai_footer_disabled_when_no_config: 設定なし時のスキップ確認
    - test_ai_footer_enabled_with_config: 設定あり時のフッタ付与確認
    - test_ai_footer_skipped_when_disabled: enabled=false時のスキップ確認
    - test_ai_footer_skipped_when_no_meta メタデータなし時のスキップ確認
  - [x] 統合テスト（CLI実行による動作確認）
    - test_cli_gen_with_ai_footer: エンドツーエンドでのフッタ付与動作確認
  - テスト実行結果: 8 passed (単体7件 + 統合1件)
- [x] 関連Issueの更新
  - メモ: フロントマターの `関連Issue` を `#291` に更新。進捗をissueに書き込むものではない。
- [x] PR 作成
  - メモ: PR #294 https://github.com/yurake/pptx_generator/pull/294（2025-11-13 完了）
