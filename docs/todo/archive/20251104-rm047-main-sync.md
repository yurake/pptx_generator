...
- [x] 計画策定（スコープ・前提の整理）
  - メモ: 
      - **対象整理**  
        ...
        - 対象: `feat/rm047-draft-structuring` ブランチ、`docs/design/archive/draft-structuring.md` ほか RM-047 関連ドキュメント。  
...
- [x] 設計・実装方針の確定
  - メモ: main 取り込み後の前提整理を完了し、`generate_ready` 基盤への移行方針は `docs/design/stages/stage-03-compose.md` 更新（commit `edfabde`）および `docs/design/design.md` 反映済み。旧 `docs/design/archive/draft-structuring.md` は参考用として保管し、再配置議論は `docs/design/stages/` 系へ統合する方針で確定。
- [x] ドキュメント更新（要件・設計）
  - メモ: main 取り込みに伴う前提差分を `docs/requirements/stages/stage-03-compose.md`・`docs/design/stages/stage-03-compose.md` 等へ反映済み（commit `edfabde`）。
  - [x] docs/requirements 配下
    - メモ: Stage3 要件を `generate_ready` 基盤へ更新（commit `edfabde`）。
  - [x] docs/design 配下
    - メモ: Stage3 設計・CLI 説明を整理（commit `edfabde`）。
- [x] 実装
  - メモ: `git merge origin/main` を実行し、`docs/design/design.md`・`docs/requirements/stages/stage-03-compose.md` の競合を main 優先で解消。`chore(merge): sync latest main into rm047` (8118ea8) を作成。
- [x] テスト・検証
...
- [x] ドキュメント更新
  ...
  - [x] docs/roadmap 配下
    - メモ: stage 3 説明および RM-024 期待成果を最新仕様へ更新（commit `c00d319`）。
  ...
  - [x] README.md / AGENTS.md
    - メモ: README stage 3 説明を刷新（commit `6a12b63`）。
- [x] 関連Issue 行の更新
  - メモ: 
- [x] PR 作成
...
## メモ
- main 側で `prepare_card.json` への名称統一、テンプレ 統合 CLI (`uv run pptx template`) が導入済み。RM-047 実装時は新ステージ構成（Stage2/Stage3/Stage4 再定義）と整合を取りながら `generate_ready` 置換を行う。
- `docs/design/archive/draft-structuring.md` が main から移動されているため、再設計版をどこへ配置するか（`docs/design/stages/` に統合する等）を設計フェーズで決定する。
