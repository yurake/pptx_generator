...
- [x] 設計・実装方針の確定
  - メモ: main 取り込み後の前提整理を完了し、`generate_ready` 基盤への移行方針は `docs/design/stages/stage-03-compose.md` 更新（commit `edfabde`）および `docs/design/design.md` 反映済み。旧 `docs/design/draft-structuring-RM047.md` は削除済みで、再配置議論は `docs/design/stages/` 系へ統合する方針で確定。
- [x] ドキュメント更新（要件・設計）
