...
## メモ
- main 側で `prepare_card.json` への名称統一、テンプレ stage 統合 CLI (`uv run pptx template`) が導入済み。RM-047 実装時は新ステージ構成（Stage2/Stage3/Stage4 再定義）と整合を取りながら `generate_ready` 置換を行う。
