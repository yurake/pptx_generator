...
- [x] 計画策定（スコープ・前提の整理）
  - メモ: 
    ...
    - 想定影響ファイル: `src/pptx_generator/cli.py`, `src/pptx_generator/prepare/orchestrator.py`, `tests/test_cli_prepare.py`, `README.md`, `docs/design/cli-command-reference.md`, `docs/requirements/stages/stage-02-prepare.md` ほか `--card-limit` 記載箇所。
...
- [x] ドキュメント更新（要件・設計）
  ...
  - [x] docs/design 配下
- [x] 実装
  - メモ: `src/pptx_generator/cli.py`, `src/pptx_generator/prepare/orchestrator.py`, `tests/test_cli_prepare.py` を `page_limit` 対応へ更新。
- [x] テスト・検証
