...
- [x] 計画策定（スコープ・前提の整理）
  - メモ: 承認済み Plan をそのまま転記する。以下の項目を含めること。
    ...
    - スコープ: RM-046「生成AIプレペア構成自動化」の最終形に合わせて工程3の仕様を再設計し、現行設計との差分を整理する。既存挙動の互換維持は考慮せず、理想像への刷新を前提とする。
    ...
    - 追記 (2025-11-02 14:05 JST): 工程4・5統合 `pptx compose` 機能の main 反映を前提に、`feat/rm046-prepare-automation` への取り込み計画を以下の通り策定済み。後方互換は考慮不要で進める。
...
- [x] ドキュメント更新
  ...
  - [x] README.md / AGENTS.md
    - メモ: README の工程3 説明から `--prepare-policy` を削除。
- [x] 関連Issue 行の更新
