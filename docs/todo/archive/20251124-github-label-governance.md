...
- [x] テスト・検証
  - メモ: `uv run python` で `.github/issue-labeler.yml` と `.github/labeler.yml` の YAML 構文を確認。GitHub Actions 手動実行時は `issue-labeler` 入力に Issue 番号が必要なため、`workflow_dispatch` に必須入力を追加済み。
- [x] ドキュメント更新
