---
目的: テンプレ抽出の jobspec スキャフォールドを stage 3 JobSpec と整合させる方針と実装をまとめる
...
- [x] テスト・検証
  - メモ: `uv run --extra dev pytest tests/test_spec_loader.py`、`uv run pptx compose samples/extract/jobspec.json --template samples/templates/templates.pptx --prepare-cards samples/prepare/prepare_card.json` を実行（当時の CLI 仕様。現在は JobSpec の `meta.template_path` にテンプレートを設定して実行する）。
- [x] ドキュメント更新
