---
目的: stage 4+5の統合CLIラッパー仕様策定と実装準備
...
- [x] 計画策定（スコープ・前提の整理）
  - メモ:
    - スコープ
      - CLI に stage 4+5を連続実行する新サブコマンド（名称: compose）を追加し、`outline` → `mapping` の再実行を一括化する。
...
- [x] ドキュメント更新
  ...
  - 追加対応計画:
    - スコープ
      - stage 体系を 6 → 5 stage へ変更し、従来の stage 4/5（ドラフト構成・マッピング）を統合して新しい「stage 4: マッピング」と定義し直す。
      - README は新 stage 4 を基準に簡潔化し、旧 stage 4/5の細部は既存の詳細ドキュメントへ退避／集約する。
      - requirements/design など stage 別ドキュメントを統合・改訂し、stage 番号と呼称を全体で整合させる。
    - 影響ドキュメント（想定）
      - `README.md`（stage 一覧・チートシート等の更新）
      - `docs/requirements/stages/*.md`（stage-04/05 の統合 → 新 stage-04）
      - `docs/design/*`（stage 解説・CLI 記述の stage 番号更新）
      - `docs/runbooks/`, `docs/notes/` など stage 数に言及する箇所全般
      - ToDo / roadmap の記録更新（必要なら）
    - 実施ステップ
      1. stage に関するドキュメントを洗い出し、6stage 前提の記述箇所を一覧化。
      2. requirements/design の stage 4/5 ドキュメントを統合し、新 stage 4「マッピング」として構成・内容を再整理。
      3. README から旧 stage 4/5 詳細を削除し、新 stage 4 概要と該当ドキュメントへの導線を記載。
      4. それ以外の資料（設計書・runbook・notes・CLI リファレンス等）の stage 数・名称・参照リンクを 5 stage 体制に合わせて更新。
      5. 変更箇所を確認し、必要に応じて diff チェックと表記揺れの統一を実施。
    - リスク・前提
      - stage 番号変更に伴う参照リンク切れ（目次・アンカー等）が発生し得る。
      - ドキュメント間で stage 呼称が混在する可能性があるため、統一ルールの適用が必要。
    ...
    - 調査メモ
      ...
      - その他 `docs/AGENTS.md`、ロードマップ、ToDo 等に散在する「stage 3/4」「stage 5/6」表現。
- [x] 関連Issue 行の更新
