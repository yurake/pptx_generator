...
## メモ
- 2025-11-12 Plan更新（ユーザー承認済み）: `supporting_point.evidence` が未定義なカードで Brief オーケストレーターが落ちないように `src/pptx_generator/brief/orchestrator.py` にガード処理を追加し、検証として `PPTX_LLM_PROVIDER=mock uv run pptx prepare ... --mode dynamic` を実行する。
