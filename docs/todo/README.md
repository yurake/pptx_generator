...
## 作成手順
1. Plan が承認されたら `rmxxx-<slug>.md` 形式で新規ファイルを作成する（例: `rm070-dev-process-guidance.md`）。コードに影響しない整備タスクでユーザーが ToDo 作成不要と明示した場合は例外的に省略できるが、判断に迷うときはユーザーへ作成要否を問い合わせる。
...
3. ロードマップ連動タスクではテンプレートに含まれる「ブランチ→計画→設計→実装→テスト→関連Issue→PR」の stage 順を維持し、各チェックボックスに完了条件を具体的に記載する。特に「設計・実装方針の確定」では、Plan を踏まえた方針メモを ToDo の該当欄へ記入し、必要なら `docs/notes/` 等へのリンクを添える。方針メモが未記入の状態で実装 stage に進まないこと。調査のみで完了する場合は、次の判断主体と判断時期を併記する。
4. メモ欄はタスク本文に収まりきらない背景・リスク・判断メモに限定し、進捗報告はチェックボックスのコメントで行う。前提/決定/リスク(UNCONFIRMED)/Now-Next/テスト実績を「連続性メモ」として短文化し、状態が変わるたび（設計確定・実装完了・テスト完了・PR 作成前後など）に上書き更新する。不要になった情報は削除し、別ファイルは作らない。

## 更新ルール
- チェックボックスは上から順に完了させ、未完のタスクが残っている状態で次の行に進まない。前段のチェックが `[x]` になっていない場合は、先に手順を完了しメモ欄を更新する。
...
- 設計・実装方針が固まったら、テンプレートの「設計・実装方針メモの共有」チェックを用いて記録場所を明示し、要件／設計ドキュメントの更新内容（または更新不要の理由）を同一 ToDo 内に残す。メモがないまま実装チェックを付けない。
- 子タスクが全て完了になった場合は、親タスクのチェック状態も即時に確認し、未完であれば `[x]` へ更新する。親タスクのメモ欄には完了内容や残タスクの有無を明記し、階層間の整合を保つ。
