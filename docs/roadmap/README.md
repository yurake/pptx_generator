...
| フェーズ | 期間 | 目的 | 主要アウトプット |
| --- | --- | --- | --- |
| フェーズA: テンプレートインテリジェンス | 2025-10-12 〜 2025-11-08 | 1・2 工程（テンプレ準備 / 構造抽出）の自動化と運用整備 | テンプレ解析 CLI、レイアウト構造 JSON、検証ツール、テンプレ運用ポリシー更新 |
| フェーズB: コンテンツ & ドラフト整備 | 2025-11-09 〜 2025-12-15 | 3・4 工程（コンテンツ正規化 / ドラフト構成）の HITL ワークフローと AI レビュー整備 | 承認 UI / API、AI レビュー実装、承認ログ連携、縮約ポリシー |
| フェーズC: マッピング & レンダリング強化 | 2025-12-16 〜 2026-01-31 | 5・6 工程（マッピング / PPTX レンダリング）の最適化と監査強化 | レイアウトスコアリング高度化、軽量整合チェック拡張、Polisher 連携、監査ログワークフロー |
...
- 次のアクション: フェーズ別タスクと紐づけて以下を進行する。
  - **フェーズA（工程1・2）**: テンプレ構造抽出 CLI 要件定義と PoC 設計（ToDo 発行予定）、テンプレ運用ポリシー更新。
  - **フェーズB（工程3・4）**: レイアウトスコアリング指標の設計、AI レビュー評価指標の数値化、承認 UI / API 設計、`docs/design/schema-extensions.md` のモデル実装。
  - **フェーズC（工程5・6）**: 監査ログ項目と承認状態遷移の最小セット定義、軽量整合チェックと Polisher 連携の拡張。
