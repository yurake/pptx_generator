...
- 対象工程: 3（マッピング）
- ゴール: JSON入力からタスクスケジュールを矢羽型ガントチャートとして自動生成し、実施時期を視覚的に表現するスライドを提供する。
- 参照ドキュメント: （作成予定：`docs/design/schedule-arrow-gantt.md`）
- 参照 ToDo: （未作成 — 着手時に `docs/todo/` へ登録）
- 状況: 未着手（2025-11-13 追加）
- 期待成果:
  - JSON入力スキーマの定義（タスク名、開始日、終了日を必須項目とし、フェーズ名を本文に含める）。
  - 矢羽型レイアウト「Schedule Arrow Gantt」の新規作成（右端が三角形、内部にテキスト配置可能な図形）。
  - 日付から自動的に矢羽の位置・長さを計算し、PowerPointオブジェクトとして配置するレンダラー拡張。
  - 複数タスクの縦並び表示、時間軸の自動調整（月次・四半期などのスケール選択対応）、凡例・ラベル配置の自動化。
- 依存: RM-038（テンプレートパターン拡充）、RM-018（レンダラー リッチコンテンツ対応）、図形生成ロジックの実装。
- 次アクション: 
  - 矢羽図形の PowerPoint 実装方式（図形結合／グループ化／カスタムシェイプ）を検討。
  - JSON スキーマ詳細設計とサンプル作成。
  - レイアウトテンプレートのアンカー定義と命名規則策定。
