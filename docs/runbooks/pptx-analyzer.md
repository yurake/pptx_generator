...
- テンプレート（スライドマスター／テーマ）が最新化され、レンダラーが図形 ID／アンカー名を付与できる状態（ブランド設定はテンプレート抽出結果の TemplateStyle/`branding.json` を参照）。
...
- `docs/design/stages/stage-02-prepare.md`: Review Engine 連携仕様と Auto-fix 対応範囲。
