...
## 背景
- 静的テンプレートではスライド順・slot が固定されるため、stage 2・stage 3 で Blueprint を共通参照し、HITL が差戻し判断を行う際に slot 充足状況を把握できるようにする。
...
- 生成カードには以下の追加メタを付与する。
  ...
  - `required`: slot の必須フラグ、stage 3 での検証用。
  - `blueprint_slot`: Blueprint 情報をそのまま埋め込み、後 stage で参照できるようにする。
...
## stage 3（`draft_structuring` / `mapping`）連携
...
- `generate_ready.json` の `slides[*].meta` へ Blueprint 情報を転記し、レンダリング stage で slot を参照できるようにする。
- `generate_ready.slides[*].meta.blueprint_slots[*].fulfilled` を保持し、stage 4 の監査とレンダリングで slot 充足状況を再利用する。
