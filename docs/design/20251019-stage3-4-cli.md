...
- ブリーフ成果物 (`prepare_card.json`, `draft_approved.json`) を事前に整備したうえで工程3 (`pptx mapping`) や工程4 (`pptx render`) を再実行できるようにする。
...
- **主なオプション**:
  ...
  - `-p/--page-limit INT`（任意）: 生成する BriefCard の上限枚数。
- **処理フロー**:
 1. `BriefSourceDocument.parse_file()` でブリーフ入力を正規化（Markdown → 章チャプター化を含む）。
 2. 既定ポリシー（`config/brief_policies/default.json`）を読み込み、`BriefAIOrchestrator.generate_document()` を呼び出す。
 3. 生成した `BriefDocument`・メタ情報・生成ログ（スタブ）を `.pptx/prepare/` 配下へ書き出す。
...
- **主なオプション**:
  - `--brief-cards PATH`（既定: `.pptx/prepare/prepare_card.json`）: ブリーフ成果物。
  - `--brief-log PATH`（既定: `.pptx/prepare/brief_log.json`）: ブリーフレビューのログ（任意）。
  - `--brief-meta PATH`（既定: `.pptx/prepare/ai_generation_meta.json`）: ブリーフ生成メタ情報（任意）。
...
- `tests/test_cli_content.py`
  - ブリーフ入力を与えて `prepare_card.json` などの成果物が生成されることを確認。
  - 無効な入力で `BriefNormalizationError` が発生するケースや `-p/--page-limit` 制御を検証。
