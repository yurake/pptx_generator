...
- 工程4 と工程5 の連続実行を標準化するため、`pptx compose` サブコマンドで `outline` → `mapping` を一括実行し、ドラフト成果物と `rendering_ready.json` の同期を保証する。
...
- `tests/test_cli_integration.py::test_cli_mapping_then_render` で `pptx mapping` → `pptx render` の再実行ルートと監査ログの `rendering_ready` ハッシュ出力を確認し、工程4/5 切り離し後もトレーサビリティが維持されることを検証する。
