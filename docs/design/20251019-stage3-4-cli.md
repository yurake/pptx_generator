...
- **処理フロー**:
 ...
 3. 生成した `BriefDocument`・メタ情報・生成ログ（スタブ）を `.pptx/content/` 配下へ書き出す。
...
- **主なオプション**:
  - `--brief-cards PATH`（既定: `.pptx/content/brief_cards.json`）: ブリーフ成果物。
  - `--brief-log PATH`（既定: `.pptx/content/brief_log.json`）: ブリーフレビューのログ（任意）。
  - `--brief-meta PATH`（既定: `.pptx/content/ai_generation_meta.json`）: ブリーフ生成メタ情報（任意）。
