...
- 承認済み成果物 (`content_approved.json`, `draft_approved.json`) を事前に整備したうえで工程4 (`pptx mapping`) や工程5 (`pptx render`) を再実行できるようにする。
...
- `tests/test_cli_integration.py::test_cli_mapping_then_render` で `pptx mapping` → `pptx render` の再実行ルートと監査ログの `generate_ready` ハッシュ出力を確認し、工程4/5 切り離し後もトレーサビリティが維持されることを検証する。
