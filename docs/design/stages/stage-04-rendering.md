...
- CLI からの実行は `uv run pptx gen .pptx/compose/generate_ready.json --branding config/branding.json` を前提とし、工程4の成果物確認には `pptx compose` / `pptx mapping` を用いる。`--output` 未指定時の出力先は `.pptx/gen/`。
