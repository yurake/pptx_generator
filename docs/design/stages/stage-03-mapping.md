...
- HITL 承認と割当ログを `generate_ready_meta.json`・`draft_review_log.json`・`mapping_log.json` に集約し、監査しやすい構造を維持する。
...
| GenerateReady Builder | プレースホルダ割付・フォールバック制御 | Python | `generate_ready.json`, `mapping_log.json` を生成 |
...
- 出力: `generate_ready.json`, `generate_ready_meta.json`, `draft_review_log.json`, `mapping_log.json`, `fallback_report.json`。

## ワークフロー概要
1. `pptx compose` が Brief 成果物とテンプレ構造を読み込み、章テンプレ辞書 (`config/chapter_templates/`) に基づいて初期章構成を作成する。
2. `CardLayoutRecommender` がカード単位でレイアウト候補を算出し、スコア内訳と共に `mapping_log.json` に記録する。Analyzer 連携がある場合は重大度情報を候補に付与する。
...
- 実行結果には Draft/Ready/Meta の各ファイルパスが表示されるため、CLI 出力を確認することでワークスペース内の最新成果物を即座に把握できる。
