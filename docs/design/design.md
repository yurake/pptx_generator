...
| マッピング (HITL + 自動) | `jobspec.json`, `brief_cards.json`, `brief_log.json`, `brief_ai_log.json`, `layouts.jsonl`, `branding.json` | `draft_draft.json` → `draft_approved.json`, `draft_meta.json`, `draft_review_log.json`, `generate_ready.json`, `mapping_log.json`, `fallback_report.json` | 章承認・差戻しログ、レイアウトスコアリング、フォールバック（縮約→分割→付録） |
