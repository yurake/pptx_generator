...
- 抽出結果は `uv run pptx layout-validate --template samples/templates/templates.pptx --output .pptx/validation/rm043` で取得し、`samples/json/sample_layouts.jsonl` と `samples/json/sample_spec.json` に反映してマッピングテストの基準データとする。
