...
## 目的
- stage 3（コンテンツ準備）でカード生成時にストーリー骨子を参照できるようにする。
- stage 4（ドラフト構成）で `story_outline` と `layout_hint` の整合を維持するための設計指針をまとめる。
- 実装前にデータフロー・API フック・UI 連携ポイントを整理し、開発タスクの分割と依存関係を明確化する。

## 背景
- 現状、stage 3 では入力コンテンツを直接スライド素材へ変換しているが、章立てやストーリーフェーズの明示がないため、stage 4 での構成調整に時間がかかっている。
- RM-005 ではストーリー設計プロセスをドキュメント化し、後続 stage で活用できるようメタデータを整備することが求められている。

## スコープ
- ドキュメント整備（企画、要件、設計）。コード実装は対象外。
- `story_outline.json` スキーマと `prepare_card.json` への拡張項目の設計。
- stage 3 UI での表示仕様、stage 4 との連携方法のたたき台作成。
...
4. 承認後の `prepare_card.json` にストーリー情報を保持し、stage 4 に渡す。
...
- `src/pptx_generator/pipeline/draft_structurer.py`
  - stage 4 で `story_outline` と `layout_hint` の整合を確認するバリデーション追加を検討。
...
- **stage 4 との齟齬:** `layout_hint` が章構成と一致しない場合、stage 4 でエラー把握できるよう警告を設計。
- **バージョン管理:** `story_outline.json` の更新履歴を残し、関連 ToDo / Issue から参照できるよう `docs/todo/` で管理。

## 次のアクション
- 要件・設計ドキュメントのレビュー後、RM-005 の実装タスクを別途切り出す。
- stage 4/5 のロードマップ更新（必要に応じて RM-024, RM-025 へストーリー連携項目を追加）。
