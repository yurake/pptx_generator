...
- `pptx template|prepare|compose|gen ... [--transaction-id <tx>]`
  - 動作: enqueue のみ。レスポンス例 `{job_id, transaction_id, status: "pending", status_hint: "pptx status --job-id <...>"}`
  - 同期実行経路は廃止。
...
- `pptx worker run [--once]`
  - pending を取得し running→終了まで処理。--once で 1 ジョブのみ実行。
...
## 移行と互換性
- 同期 CLI を廃止。既存スクリプトは `pptx <stage> ...` の呼び出し結果が job_id を返す点を周知する。
