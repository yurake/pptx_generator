...
- LLM ログ: 人向けログ（stdout/ファイル）にはプロンプト/レスポンス本文をそのまま出力してよい。監査ログ（例: `prepare_ai_log.json`）にも記録する。PII/機微情報が含まれる可能性があるため、運用側でログ保護・アクセス制御を行う。
...
- 生成 AI 呼び出し: 呼び出し開始と完了時にモデル名・ステータス・レイテンシを INFO/WARN で出す。人向けログではプロンプト/レスポンス本文を出力してよい。PII/機微情報が含まれる可能性があるため、運用でのログ保護と取り扱いに注意する。監査ログ（例: `prepare_ai_log.json`）には引き続き記録する。
