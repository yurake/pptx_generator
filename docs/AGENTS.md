...
- `config/`: CLI が参照する設定を配置（例: `rules.json`）。usage_tags の canonical 定義は `src/pptx_generator/config/usage_tags.json` に集約されている（環境変数 `PPTX_GENERATOR_USAGE_TAGS` で上書き可能）。更新時は対応する requirements/design/runbook へ根拠を記録する。
