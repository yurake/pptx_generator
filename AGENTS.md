...
- Plan を提示する前に、対象作業の ToDo に目的・関連ブランチ・roadmap_item が記録され、ブランチ作成と初期コミットが完了していること、およびチェックリストが工程を網羅していることを確認する。Plan 承認後は内容を ToDo の「計画策定」チェック項目のメモ欄へ全文転記し、Plan と ToDo の整合を維持する。計画のみで完了する判断をする場合は、判断者・判断日・次の判断条件を ToDo のメモ欄に追記し、再開時に工程チェックをやり直す。PR 前には `gh issue list --limit 50` 等で関連 Issue 番号を確認し、ToDo の「関連Issueの更新」チェックで最新状況を記録する。
- 実装に着手する前に ToDo の「設計・実装方針の確定」で方針メモを更新し、必要に応じて `docs/notes/` などへ詳細を記録してリンクする。方針メモが未記入の状態で実装工程へ進まないこと。
