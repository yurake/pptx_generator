...
# Static template → prepare → compose → gen を一気通貫で実行し、
...
TEMPLATE_PATH=${TEMPLATE_PATH:-samples/templates/static_slide.pptx}
...
tpl_resp=$(curl_json POST "${BASE_URL}/templates" \
  -d "{\"template_path\":\"${TEMPLATE_PATH}\",\"mode\":\"static\"}")
echo "$tpl_resp"
...
if curl -sS "$BASE_URL/jobs/$gen_job/artifacts/pptx" -H "Authorization: Bearer $TOKEN" -o "/tmp/${tx}_static.pptx"; then
  size=$(stat -f%z "/tmp/${tx}_static.pptx" 2>/dev/null || stat -c%s "/tmp/${tx}_static.pptx" 2>/dev/null)
