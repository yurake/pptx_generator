...
| 2. テンプレ構造抽出 | 自動 | テンプレートファイル | `jobspec.json`, `branding.json` | テンプレからジョブスペック雛形と layout-style 定義を生成 |
| 3. コンテンツ正規化 | HITL | `jobspec.json`, 入力コンテンツ | `content_approved.json` | 入力データをスライド候補へ整形し、承認を行う |
| 4. ドラフト構成設計 | HITL | `jobspec.json`, `content_approved.json` | `draft_approved.json` | 章立て・ページ順・`layout_hint` を確定し、承認を行う |
| 5. マッピング | 自動 | `jobspec.json`, `draft_approved.json` | `rendering_ready.json` | レイアウト選定とプレースホルダ割付を行い、中間 JSON を生成 |
...
flowchart TD
  ...
  %% ======= Stage 3 =======
  Jobspec --> S3["**工程 3 コンテンツ正規化 (HITL)**"]:::stage
...
| 4. ドラフト構成設計 | `uv run pptx outline .pptx/extract/jobspec.json --content-approved .pptx/content/content_approved.json --output .pptx/draft` | `.pptx/draft/draft_approved.json` | 章立てとページ順を確定し、承認成果物を出力 |
| 5. マッピング | `uv run pptx mapping .pptx/extract/jobspec.json --content-approved .pptx/content/content_approved.json --layouts .pptx/validation/layouts.jsonl --draft-output .pptx/draft --branding .pptx/extract/branding.json --output .pptx/gen` | `.pptx/gen/rendering_ready.json` | `.pptx/draft/draft_approved.json` を再生成しつつレイアウト割り付け |
...
| 工程3〜6 一括 | `uv run pptx gen .pptx/extract/jobspec.json --template samples/templates/templates.pptx --branding .pptx/extract/branding.json --output .pptx/gen --export-pdf` | `.pptx/gen/proposal.pptx`（`proposal.pdf` 任意） | パイプライン後半をまとめて実行 |
...
### 工程 2: テンプレ構造抽出
- テンプレート PPTX からレイアウトとアンカー情報を抽出し、工程3以降で参照する `jobspec.json` と補助成果物（`layouts.jsonl`・`branding.json`）を生成します。
- 差分チェックや品質検証には `pptx layout-validate` を併用します。
- 詳細ガイド: `docs/requirements/stages/stage-02-template-structure-extraction.md`

### 工程 3: コンテンツ正規化
- 工程2で生成した `jobspec.json` を基に入力コンテンツをスライド候補へ整形し、HITL で `content_approved.json` を確定します。生成AIドラフトの作成やレビュー支援は `pptx content` コマンドで行い、生成AIモードが既定です。
...
### 工程 4: ドラフト構成設計
- `jobspec.json` と承認済みコンテンツを基に章立てやページ順を確定し、HITL で `draft_approved.json` を承認します。
...
- 承認済みコンテンツからドラフト成果物を生成する場合は `pptx outline`（新名称）を利用します。
  ```bash
  uv run pptx outline .pptx/extract/jobspec.json \
...
### 工程 5: マッピング
- `jobspec.json`・`draft_approved.json`・承認済みコンテンツを入力にレイアウトスコアリングとフォールバック制御を行い、`rendering_ready.json`・`mapping_log.json`・必要に応じて `fallback_report.json` を生成します。`mapping_log.json` には Analyzer 指摘サマリ（件数集計・スライド別詳細）が追加されており、補完やフォールバック制御の判断材料として活用します。詳細は `docs/requirements/stages/stage-05-mapping.md` と `docs/design/stages/stage-05-mapping.md` を参照してください。
- 実行手順:
  ```bash
  uv run pptx mapping .pptx/extract/jobspec.json \
    --content-approved .pptx/content/content_approved.json \
