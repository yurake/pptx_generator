...
2. `uv run pptx compose samples/json/sample_jobspec.json --template samples/templates/templates.pptx --output .pptx/compose` で `generate_ready.json` を用意し、`uv run pptx gen .pptx/compose/generate_ready.json --branding config/branding.json --output .pptx/gen` などで出力を確認し、テーブル・チャート・テキストボックスのスタイルが意図通り反映されることを確認する。
