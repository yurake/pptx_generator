...
## ファイル概要
- 主要設定はパッケージ同梱の `src/pptx_generator/config/pipeline_rules.json` を使用する（リポジトリ直下には置かない）。
...
3. 必要に応じて `docs/policies/config-and-templates.md` に理由・検証手順を追記する（テンプレートや関連設定を更新した場合は必須）。
...
## バリデーション
- ルール設定を変更する際は `tests/test_analyzer.py` や `tests/test_refiner.py` の影響を確認する。
