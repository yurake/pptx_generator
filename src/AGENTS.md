...
## 構成概要
- エントリーポイント: `pptx_generator/cli.py`（`pyproject.toml` の `pptx-generator` スクリプト）。
...
- パイプライン全体: `uv run pptx-generator run samples/json/sample_spec.json --workdir .pptxgen` を実行し、`outputs/audit_log.json` の差分を確認。
