...
from schedule.schedule_parser import parse_schedule_markdown
...
def parse_personnel_xlsx(xlsx_path: str | Path) -> PersonnelData:
    ...
    # 期待する列構造:
    # 1: プロジェクト, 2: 工程(フェーズ), 3: 作業内容(タスク), 4: 年, 5: 月,
    # 6: 正社員(人月), 7: パートナー(人月), 8: 外注(人月)
    phases: dict[str, PersonnelPhaseData] = {}
    for row_idx in range(2, ws.max_row + 1):
        project = ws.cell(row=row_idx, column=1).value
        phase_name = ws.cell(row=row_idx, column=2).value
        ...
        emp = ws.cell(row=row_idx, column=6).value
        pn = ws.cell(row=row_idx, column=7).value
        si = ws.cell(row=row_idx, column=8).value

        if not phase_name or not task_name or not year or not month:
            continue
        ...
        # 当該タスクの既存を探す（行が月ごとに分かれているためマージ）
        existing = next((t for t in phases[phase_name].tasks if t.task_name == task_name), None)
        if existing is None:
            existing = PersonnelTaskData(task_name=task_name, months=[])
            phases[phase_name].tasks.append(existing)

        # 月別工数を追加（0やNoneはスキップせず0扱いで追加し、期間決定に使う）
        def _val(v):
            try:
                return float(v or 0)
            except Exception:
